...
export async function fetchNotionPageBlocks(id, from = null, slice = 0) {
  ...
  const hydratedPageBlock = await hydrateMissingPageBlocks(pageBlock)
  if (hydratedPageBlock) {
...
export function formatNotionBlock(block) {
  ...
  const validBlockIds = new Set(Object.keys(clonedBlock || {}))
  ...
  for (let i = 0; i < blocksToProcess.length; i++) {
    ...
    if (
      ['header_4', 'header_5', 'header_6'].includes(b?.value?.type)
    ) {
      b.value.type = 'sub_sub_header'
    }
    ...
    if (Array.isArray(b?.value?.content)) {
      b.value.content = b.value.content.filter(childId =>
        validBlockIds.has(childId)
      )
    }

    if (Array.isArray(b?.value?.children)) {
      b.value.children = b.value.children.filter(childId =>
        validBlockIds.has(childId)
      )
    }
  }
...
async function hydrateMissingPageBlocks(pageBlock) {
  if (!pageBlock?.block || typeof pageBlock.block !== 'object') {
    return pageBlock
  }

  const hydratedPageBlock = {
    ...pageBlock,
  ...
  while (true) {
    const missingIds = collectMissingChildIds(hydratedPageBlock.block).filter(
    ...
    fetchedKeys.forEach(id => fetchedIds.add(id))
    hydratedPageBlock.block = {
      ...hydratedPageBlock.block,
      ...fetchedBlocks
    }
  }
...
function collectMissingChildIds(blockMap) {
  const existingIds = new Set(Object.keys(blockMap || {}))
  const missingIds = new Set()

  Object.values(blockMap || {}).forEach(block => {
    const value = unwrapBlockValue(block)
    ;['content', 'children'].forEach(field => {
      if (!Array.isArray(value?.[field])) {
        return
      }
      value[field].forEach(childId => {
...
function unwrapBlockValue(block) {
